...
class Simulation {
	...
	private int takenAction = 0;
	
	// reward attributes for wumpus__________________________________________________________________________
	// they can be changed only from here!
	private static int killReward = 1000;
	
	private int wcurrScore = 0;
	private int wstepCounter = 1;
	...
	private Agent agent;
	private InnerPerceivedData pd;
	
	private WumpusAgent wumpus;
	private WumpusInnerPerceivedData wpd;
	...
	public Simulation(Environment E, int maxSteps) {
		...
		System.out.println("Initial evironment: ");
		...
		while (true) {
			...
			if (agent.IAmDead() && wumpus.IKilledIt()) {
				System.out.println("\nAgent is killed by the Wumpus!!");
				break;
			}
			if (agent.IAmDead() && !wumpus.IKilledIt()) {
				System.out.println("\nAgent fell into a bottomless pit!!");
				break;
			}
	...
	public void handleWumpusAction (int action) {
		...
		// must be careful when moving forward ______________________________________________________________
		if (action == WumpusActionSet.MOVE_FORWARD) {
			
			System.out.println("Moving forward...");
			// save the action for display later
			wtakenAction = WumpusActionSet.MOVE_FORWARD;
			// induce the cost
			wcurrScore += actionCost;
			// let the wumpus move forward
			// but if it is about to fall out
			// of the world
			if (wumpus.moveForward() == false) {
				// generate force field and zap the agent
				// no need to update the agent's location
			...
			// if moving forward kills the wumpus
			if (environment.wumpusIsDead()) {
				// induce death cost, let the wumpus scream, and self-terminate
		...
		// simply turn right ________________________________________________________________________________
		else if (action == WumpusActionSet.TURN_RIGHT) {
			
			System.out.println("Turning right...");
			// save the action for display later
			wtakenAction = WumpusActionSet.TURN_RIGHT;
			// induce the cost
			wcurrScore += actionCost;
			// let the agent turn right
			wumpus.turnRight();
		...
		// simply turn left _________________________________________________________________________________
		else if (action == WumpusActionSet.TURN_LEFT) {
			
			System.out.println("Turning left...");
			// save the action for display later
			wtakenAction = WumpusActionSet.TURN_LEFT;
			// induce the cost
			wcurrScore += actionCost;
			// let the agent turn left
			wumpus.turnLeft();
		...
		// body slam ________________________________________________________________________________________
		else if (action == WumpusActionSet.SLAM) {
			
			System.out.println("Jump...BAAM!!");
			// save the action for display later
			wtakenAction = WumpusActionSet.SLAM;
			// if the wumpus can swat the agent
			if (environment.wumpusSlamAgent() == true) {
				System.out.println("Wumpus think he swatted something!");
				// give the wumpus reward and turn of the simulation
				wumpus.killIt();
