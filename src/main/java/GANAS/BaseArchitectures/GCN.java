...
public class GCN {
    ...
    public SimpleMatrix forward(int t, SimpleMatrix A_hat, SimpleMatrix X) { // [nodes_indices x n_features]
        ...
        // DOMAIN SPECIFIC: scale down X[3:4] because coordinates are huge values
        X = X.cols(0, 3).concatColumns(X.cols(3, 5).scale(0.01), X.cols(5, 6));
    ...
    public void BPTT(int t, SimpleMatrix dL_dG) { // [1 x n_outputs]
        // in this application, GCN is always the input-end so no
        // need to return back gradient, ie dL/dX

        SimpleMatrix dL_dH = graphAgg.backward_dL_dY(t, dL_dG); // [1 x (node_indices * n_features)]
