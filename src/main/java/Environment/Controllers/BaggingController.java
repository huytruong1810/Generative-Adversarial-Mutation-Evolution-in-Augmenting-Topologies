...
public class BaggingController extends Controller {

    private int t; // tile size
    private ArrayList<Individual> humanBrains;
    private Environment E;
    private EnsembleRoom S;
    ...
    @FXML public void restart() {
        resetPrep();
        makeBaggingScene(primaryStage, S, bluePrint, humanBrains);
    }

    @FXML public void newMap() {
        resetPrep();
        makeBaggingScene(primaryStage, S, null, humanBrains);
    }
    ...
    @FXML public void runNextStep() {
        ...
        hProgress.getItems().add("Step " + S.getStepCounter() + String.format("\nScore: %d Agreed Action: %s", S.getHScore(), S.getHAction()));
        wProgress.getItems().add("Step " + S.getStepCounter() + String.format("\nScore: %d Agreed Action: %s", S.getWScore(), S.getWAction()));

        hProgress.getItems().add("Choices " + S.getHChoices() + "\nState values " + S.getHStateValues());
    ...
    public void makeBaggingScene(Stage s, EnsembleRoom ensembleRm, char[][][] bp, ArrayList<Individual> hBrains) {
        ...
        S.factoryReset();
        for (Individual hBrain : humanBrains) {
            Button btn = new Button(String.valueOf(hBrain.getScore()));
            btn.setTooltip(Utils.customTooltip(hBrain.toString()));
            btn.setOnAction(e -> Utils.inspectPopUp(primaryStage, "Member " + hBrain.getScore(), hBrain.toString()));
            MembersBox.getChildren().add(btn);
            S.reset(null, hBrain, null);
        }
        S.completeSetup(bluePrint);
        ...
        for (int i = 0; i < worldSize; ++i) {
            ...
            for (int j = 0; j < worldSize; ++j) {
                ...
                double jt = j*t;
                if (Math.random() < 0.9) gc.drawImage(new Image(Objects.requireNonNull(getClass().getResource("/images/tree" + (int) (Math.random() * 11) + ".png")).toExternalForm()), jt+t1, it+t1);
                ...
                if (Math.random() < 0.3) gc.drawImage(new Image(Objects.requireNonNull(getClass().getResource("/images/rock" + (int) (Math.random() * 8) + ".png")).toExternalForm()), jt+t1, it+t2);
                ...
                if (Math.random() < 0.3) gc.drawImage(new Image(Objects.requireNonNull(getClass().getResource("/images/rock" + (int) (Math.random() * 8) + ".png")).toExternalForm()), jt+t2, it+t1);
                ...
                if (Math.random() < 0.9) gc.drawImage(new Image(Objects.requireNonNull(getClass().getResource("/images/tree" + (int) (Math.random() * 11) + ".png")).toExternalForm()), jt+t2, it+t2);
